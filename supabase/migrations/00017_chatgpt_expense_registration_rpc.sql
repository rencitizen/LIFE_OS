-- ChatGPT expense registration MVP.
-- Provides one transaction-safe entry point that creates the expense,
-- snapshots settlement splits when required, and writes an audit log.
...
begin
  ...
  -- Authenticated app callers may only register their own payment.
  -- Privileged database callers (used by the ChatGPT connector) have auth.uid() = null.
